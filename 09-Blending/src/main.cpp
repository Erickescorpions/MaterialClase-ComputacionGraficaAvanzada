...
// Shader para mostrar solo una textura
...
// ordenar  objetos con transparencia
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	textureScreen.loadImage();
	glGenTextures(1, &textureScreenID);
...
bool processInput(bool continueApplication) {
	...
	if(!iniciaPartida) {
		...
		if(textureActivaID == textureInit1ID && presionarEnter) {
			...
			textureActivaID = textureActivaID == textureInit1ID ? textureInit2ID : textureInit1ID;
...
void applicationLoop() {
	...
	while (psi) {
		...
		// Actualizando posicion de objetos con transparencia
		// modelMatrixAircraft[3]: tiene la posicion del modelo
		...
		// Ordenar los objetos respecto a la distancia del observador
		...
		for(; itUnsorted != blendingUnsorted.end(); itUnsorted++) {
			// obtenemos la distancia de la camara a la posicion del modelo
		...
		// render de los objetos con transparencia
		// activamos el blending
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		glDisable(GL_CULL_FACE);

		std::map<float, std::pair<std::string, glm::vec3>>::reverse_iterator itSorted = blendingSorted.rbegin();
		for(; itSorted != blendingSorted.rend(); itSorted++) {
			if(itSorted->second.first.compare("aircraft") == 0) {
			...
			} else if(itSorted->second.first.compare("lambo") == 0) {
			...
			} else if(itSorted->second.first.compare("heli") == 0) {
		...
		modelText->render("Hola mundo (:", -1, 0, color, 20);
